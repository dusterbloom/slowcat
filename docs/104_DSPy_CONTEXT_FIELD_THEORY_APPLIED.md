# Context Field Engineering Implementation Guide for SlowCat

## Executive Summary

This guide outlines the integration of davidkimai's Context Engineering field-theoretic approach into SlowCat, replacing the current symbol-based system with a deterministic, measurable context field system. This will enable reproducible context generation, quantifiable optimization, and systematic evaluation while maintaining the 4096-token budget constraint for qwen3:1.7b.
...
- `/server/memory/dynamic_tape_head.py` - Tape storage and retrieval logic (repurpose for field-based retrieval)
- `/server/processors/smart_context_manager.py` - Context management pipeline (extend with field system)
- `/server/core/prompt_builder.py` - Prompt assembly (replace with field compiler)
- `/server/config/` - Configuration system (add field configurations)

**Components to Replace:**
- Symbol-based scoring in `dynamic_tape_head.py`
- Non-deterministic context assembly in `prompt_builder.py`
...
```
┌─────────────────────────────────────────────────────────────┐
│                     SlowCat Context Pipeline                 │
├───────────────────────────────────────────────────────────────┤
│                                                               │
│  Input Layer          Field Layer         Output Layer       │
│  ┌──────────┐        ┌──────────┐       ┌──────────┐       │
│  │  Query   │───────▶│  Field   │──────▶│ Context  │       │
│  │ Analyzer │        │ Computer │       │ Compiler │       │
│  └──────────┘        └──────────┘       └──────────┘       │
│       │                    │                   │             │
│       ▼                    ▼                   ▼             │
│  ┌──────────┐        ┌──────────┐       ┌──────────┐       │
│  │ History  │───────▶│  Tier    │──────▶│  Token   │       │
│  │ Processor│        │Activator │       │ Manager  │       │
│  └──────────┘        └──────────┘       └──────────┘       │
│                                                               │
│  Metrics Layer                                               │
│  ┌────────────────────────────────────────────────┐         │
│  │  Reproducibility │ Efficiency │ Quality        │         │
│  └────────────────────────────────────────────────┘         │
└───────────────────────────────────────────────────────────────┘
```
...
#### 1.1 Create Core Context Field Module

**File:** `/server/slowcat/context/context_field.py`

```python
"""
Context Field System for SlowCat
Based on davidkimai/Context-Engineering field theory
References: 
- Context-Engineering/00_foundations/03_cells_memory.md
- Context-Engineering/00_foundations/06_neural_fields.md
"""

# Core implementation as outlined in previous response
# See: ContextAtom, ContextMolecule, ContextCell, ContextField classes
```

**Deliverables:**
- [ ] Core field computation engine
- [ ] Deterministic vector generation
- [ ] Tier activation system
- [ ] Metrics collection

**Integration Points:**
- Import `get_token_counter()` from `/server/processors/token_counter.py`
- Read configuration from `/server/config/context_field.json`

#### 1.2 Configuration Schema

**File:** `/server/config/context_field.json`

```json
{
  "version": "1.0",
  "description": "Context Field Configuration for SlowCat",
  "reference": "davidkimai/Context-Engineering field theory",
  ...
  "tiers": {
    "core": {
      "tokens": 500,
      "atoms": ["IDENTITY", "CAPABILITY"],
      "weight": 1.0,
      "templates": {
        "IDENTITY": "You are SlowCat, a friendly local voice AI assistant running entirely on macOS.",
        "CAPABILITY": "You can recognize speakers, use tools, and maintain conversation context."
      }
    },
    "dynamic": {
      "tokens": 2000,
      "atoms": ["STATE", "MEMORY", "CONTEXT"],
      "weight": 0.8
    },
    "tools": {
      "tokens": 1500,
      "atoms": ["TOOLS", "EXAMPLES"],
      "weight": 0.6
    }
  },
  ...
  "field_parameters": {
    "distance_decay": 0.9,
    "resonance_boost": 1.2,
    "interference_penalty": 0.7,
    "activation_threshold": 0.1
  },
  
  "evaluation_metrics": {
    "token_efficiency_target": 4000,
    "tier_balance_variance_threshold": 0.25,
    "field_coherence_cv_threshold": 0.5
  }
}
```
...
#### 2.1 Modify SmartContextManager
...
```python
# Line ~25: Add imports
from slowcat.context.context_field import ContextField, ContextMetrics

# Line ~95: Add to __init__
self.use_context_field = os.getenv('USE_CONTEXT_FIELD', 'false').lower() == 'true'
if self.use_context_field:
    self.context_field = ContextField()
    self.context_metrics = ContextMetrics()
    logger.info("Context Field system initialized")

# Line ~150: Replace/extend _generate_context method
async def _generate_context_field(self, user_input: str) -> str:
    """Generate context using field theory"""
    history = self._get_recent_history()
    intent = self._classify_intent_deterministic(user_input)
    
    context, metrics = self.context_field.assemble_context(
        query=user_input,
        history=history,
        intent=intent
    )
    ...
    # Log for monitoring
    logger.debug(f"Field metrics: {metrics}")
    
    return context

# Add new method for deterministic intent classification
def _classify_intent_deterministic(self, query: str) -> str:
    ...
    if any(word in query_lower for word in ['hi', 'hello', 'hey']):
        ...
        return 'question'
    elif any(word in query_lower for word in ['play', 'stop', 'skip']):
        return 'command'
    else:
        return 'statement'
```

#### 2.2 Create Migration Adapter

**File:** `/server/slowcat/context/migration_adapter.py`

```python
"""
Adapter for migrating from symbol-based to field-based context
Allows gradual rollout and A/B testing
"""

class ContextMigrationAdapter:
    def __init__(self, tape_head, context_field):
        self.tape_head = tape_head
        self.context_field = context_field
        self.use_field_percentage = 0.0  # Start with 0% field usage
        
    def generate_context(self, query, history, mode='hybrid'):
        if mode == 'symbol':
            return self._generate_symbol_context(query, history)
        elif mode == 'field':
            return self._generate_field_context(query, history)
        elif mode == 'hybrid':
            return self._generate_hybrid_context(query, history)
    
    def _generate_hybrid_context(self, query, history):
        """Blend symbol and field approaches"""
        # Get both contexts
        symbol_context = self._generate_symbol_context(query, history)
        field_context, metrics = self.context_field.assemble_context(query, history)
        
        # Blend based on percentage
        if random.random() < self.use_field_percentage:
            return field_context
        else:
            return symbol_context
```

### Phase 3: DSPy Integration (Week 2)

#### 3.1 DSPy Compiler Module

**File:** `/server/slowcat/context/dspy_compiler.py`

```python
"""
DSPy-based optimization for Context Fields
References:
- DSPy documentation
- Context-Engineering/00_foundations/07_cognitive_tools.md
"""

import dspy
from slowcat.context.context_field import ContextField

class FieldSignature(dspy.Signature):
    """Signature for field optimization"""
    query: str = dspy.InputField()
    field_vector: str = dspy.InputField()
    history_summary: str = dspy.InputField()
    
    tier_weights: str = dspy.OutputField(desc="Optimized tier activation weights")
    
class OptimizedContextField(dspy.Module):
    def __init__(self, base_field: ContextField):
        super().__init__()
        self.base_field = base_field
        self.optimizer = dspy.ChainOfThought(FieldSignature)
        
    def forward(self, query: str, history: list) -> tuple:
        # Compute base field
        field_vector = self.base_field.compute_field_strength(query, history)
        
        # Optimize with DSPy
        result = self.optimizer(
            query=query,
            field_vector=str(field_vector.tolist()),
            history_summary=self._summarize_history(history)
        )
        
        # Parse and apply weights
        weights = self._parse_weights(result.tier_weights)
        return self.base_field.assemble_with_weights(query, history, weights)
```

#### 3.2 Training Pipeline

**File:** `/server/scripts/train_context_field.py`

```python
#!/usr/bin/env python3
"""
Train Context Field with conversation logs
Usage: python train_context_field.py --logs data/conversation_logs/
"""
...
import dspy
from slowcat.context.dspy_compiler import OptimizedContextField
from slowcat.context.context_field import ContextField

def load_conversation_logs(log_dir: Path):
...
def create_training_examples(logs):
    """Convert logs to DSPy training examples"""
    examples = []
    for log in logs:
        ...
        example = dspy.Example(
            ...
            expected_context=log.get("context", ""),
            quality_score=log.get("quality", 0.5)
        ).with_inputs("query", "history")
        examples.append(example)
    return examples

def train_field_optimizer(examples):
    """Train the field optimizer"""
    # Initialize models
    base_field = ContextField()
    optimizer = OptimizedContextField(base_field)
    ...
    def field_metric(gold, pred, trace=None):
        """Evaluate field quality"""
        # Check token compliance
        if pred.total_tokens > 4096:
            return 0.0
        
        # Check tier balance
        balance_score = pred.tier_balance
        
        # Check reproducibility
        repro_score = 1.0 if pred.is_deterministic else 0.0
        
        return (balance_score + repro_score) / 2
    
    bootstrap = BootstrapFewShot(metric=field_metric)
    compiled_field = bootstrap.compile(optimizer, trainset=examples[:50])
    
    return compiled_field

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--logs", required=True, help="Log directory")
    parser.add_argument("--output", default="models/context_field.pkl")
    args = parser.parse_args()
    
    # Load and train
    logs = load_conversation_logs(Path(args.logs))
    examples = create_training_examples(logs)
    optimized_field = train_field_optimizer(examples)
    
    # Save model
    dspy.save(optimized_field, args.output)
    print(f"Saved optimized field to {args.output}")
```
...
**File:** `/server/tests/test_context_field.py`

```python
"""
Test suite for Context Field system
Ensures determinism, efficiency, and quality
"""
...
class TestContextField:
    ...
    def test_determinism(self):
        ...
        context1, metrics1 = field.assemble_context(query, history)
        context2, metrics2 = field.assemble_context(query, history)
        
        assert context1 == context2
        assert metrics1["reproducibility_hash"] == metrics2["reproducibility_hash"]
    
    def test_token_budget(self):
        """Must stay within 4096 tokens"""
        field = ContextField()
        ...
        context, metrics = field.assemble_context(query, history)
        
        assert metrics["total_tokens"] <= 4096
    
    def test_field_vector_dimensions(self):
        """Field vector must have consistent dimensions"""
        field = ContextField()
        
        vector = field.compute_field_strength("test", [])
        assert vector.shape == (10,)  # Fixed 10 dimensions
        assert np.all(vector >= 0) and np.all(vector <= 1)
    
    def test_tier_activation(self):
        """Tiers should activate based on context"""
        field = ContextField()
        
        # Question should activate core tier more
        _, metrics_q = field.assemble_context("What is this?", [])
        
        # Command should activate tools tier more  
        _, metrics_c = field.assemble_context("Play music", [])
        
        assert metrics_q["tier_activations"]["core"] > 0
        assert metrics_c["tier_activations"]["tools"] > 0
```

#### 4.2 A/B Testing Framework

**File:** `/server/slowcat/context/ab_test.py`

```python
"""
A/B testing framework for comparing context systems
"""

class ContextABTest:
    def __init__(self, system_a, system_b):
        self.system_a = system_a  # Symbol-based
        self.system_b = system_b  # Field-based
        self.results = []
        
    def run_test(self, query, history, measure_response=None):
        """Run both systems and compare"""
        # Generate contexts
        context_a = self.system_a.generate(query, history)
        context_b, metrics_b = self.system_b.assemble_context(query, history)
        
        # Measure if response evaluator provided
        result = {
            "query": query,
            "context_a_length": len(context_a),
            "context_b_length": len(context_b),
            "context_b_metrics": metrics_b
        }
        
        if measure_response:
            result["quality_a"] = measure_response(context_a)
            result["quality_b"] = measure_response(context_b)
    ...
    def analyze_results(self):
        ...
        analysis = {
            "total_tests": len(df),
            "avg_length_a": df["context_a_length"].mean(),
            "avg_length_b": df["context_b_length"].mean(),
            "reproducibility_b": 1.0,  # Field system is always reproducible
            "token_efficiency_b": df["context_b_metrics"].apply(
                lambda x: x.get("token_usage", {}).get("total", 0)
            ).mean() / 4096
        }
        
        if "quality_a" in df.columns:
            analysis["avg_quality_a"] = df["quality_a"].mean()
            analysis["avg_quality_b"] = df["quality_b"].mean()
            
            # Statistical significance test
            from scipy.stats import ttest_rel
            t_stat, p_value = ttest_rel(df["quality_a"], df["quality_b"])
...
```bash
# Context Field System
USE_CONTEXT_FIELD=false  # Enable context field system
CONTEXT_FIELD_MODE=hybrid  # Options: symbol, field, hybrid
CONTEXT_FIELD_PERCENTAGE=0.0  # Percentage of field usage in hybrid mode
CONTEXT_FIELD_LOG_METRICS=true  # Log metrics for analysis
CONTEXT_FIELD_CONFIG_PATH=config/context_field.json
```

#### 5.2 Monitoring Dashboard

**File:** `/server/monitoring/context_metrics.py`

```python
"""
Real-time monitoring for Context Field system
"""
...
class ContextMonitor:
    ...
    def log_metrics(self, metrics: dict):
        """Log metrics from context generation"""
    ...
    def get_summary(self):
        ...
        token_usages = [m["total_tokens"] for m in self.metrics_buffer]
        field_coherences = [m.get("field_coherence", 0) for m in self.metrics_buffer]
        
        return {
            "avg_tokens": np.mean(token_usages),
            "max_tokens": np.max(token_usages),
            "min_tokens": np.min(token_usages),
            "avg_coherence": np.mean(field_coherences),
            "total_contexts": len(self.metrics_buffer),
            "reproducibility": 1.0  # Always 1.0 for field system
        }
```
...
### Week 1: Foundation
1. Implement core Context Field system
2. Create configuration schema
3. Write unit tests for determinism

### Week 2: Integration
1. Integrate with SmartContextManager
2. Implement migration adapter
3. Add DSPy optimization layer
4. Create training pipeline

### Week 3: Evaluation & Deployment
1. Run A/B tests comparing systems
2. Set up monitoring dashboard
3. Gradual rollout using feature flags:
   - Start with 0% field usage
   - Increase by 10% daily if metrics are positive
   - Full rollout once confidence is established

## Success Metrics

1. **Determinism**: 100% reproducibility on same inputs
2. **Token Efficiency**: Stay within 4096 tokens 100% of time
3. **Response Quality**: Equal or better than symbol system
4. **Latency**: < 50ms overhead vs current system
5. **Evaluation Scores**:
...
1. **Rollback Plan**: Feature flags allow instant reversion
2. **Hybrid Mode**: Gradual transition reduces risk
3. **Comprehensive Testing**: Unit, integration, and A/B tests
4. **Monitoring**: Real-time metrics for quick issue detection

## References

- davidkimai/Context-Engineering: Foundation for field theory
- DSPy Documentation: Optimization framework
- SlowCat Architecture: Existing codebase structure
- Token counting: `/server/processors/token_counter.py`
- Memory system: `/server/memory/` modules

## Next Steps

1. **Immediate**: Review and approve architecture
2. **Day 1-2**: Implement Phase 1 (Foundation)
3. **Day 3-5**: Implement Phase 2 (Integration)
4. **Day 6-7**: Implement Phase 3 (DSPy)
5. **Week 2**: Testing and evaluation
6. **Week 3**: Gradual production rollout

This architecture provides a clear path from SlowCat's current symbol-based system to a deterministic, field-based approach that is measurable, optimizable, and reproducible.
